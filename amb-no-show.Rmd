...
no_show_specialty <- scheduling_data_raw %>%
  filter(Campus %in% sites) %>%
  filter(Appt.Status %in% c("Arrived", "NoShow", "SameDayCanceled")) %>%
  group_by(Campus, Campus.Specialty.Group, Appt.Year, Appt.MonthYear, New.PT2, wait_time_type, Appt.Status) %>%
  summarise(total = n()) %>%
...
# Add Missing ApptStatusFinal
  nms <- c("Campus", "Campus.Specialty.Group", "Appt.Year", "Appt.MonthYear", "New.PT2",
          "Arrived", "Arrived_belowThirty", "Arrived_overThirty",
          "NoShow", "NoShow_belowThirty", "NoShow_overThirty",
          "SameDayCanceled", "SameDayCanceled_belowThirty", "SameDayCanceled_overThirty")   # Vector of columns you want in this data.frame
...
no_show_specialty <- no_show_specialty %>%
  mutate(`Arrived` = `Arrived_belowThirty` + `Arrived_overThirty`,
         `NoShow` = `NoShow_belowThirty` + `NoShow_overThirty`,
         `SameDayCanceled` = `SameDayCanceled_belowThirty` + `SameDayCanceled_overThirty`)
...
no_show_dept <- scheduling_data_raw %>%
  filter(Campus %in% sites) %>%
  filter(Appt.Status %in% c("Arrived", "NoShow", "SameDayCanceled")) %>%
  group_by(Campus, Campus.Specialty.Group, Department, Appt.Year, Appt.MonthYear, New.PT2, wait_time_type, Appt.Status) %>%
  summarise(total = n()) %>%
  pivot_wider(names_from = Appt.Status:wait_time_type,
              values_from = total,
              values_fill = 0) 
...
htmltools::browsable(
  ...
  reactable(
    ...
    colGroup(name = paste0(last_year, " YTD Total Volume"), 
             columns = c("Mon_total_day", "Tue_total_day", "Wed_total_day", "Thu_total_day", "Fri_total_day"),
             headerStyle = list(color = "#212070", fontSize = "16px")),
    colGroup(name = paste0(last_year, " YTD Avg Volume per Day"), 
             columns = c("Mon_avg_day", "Tue_avg_day", "Wed_avg_day", "Thu_avg_day", "Fri_avg_day"),
             headerStyle = list(color = "#7f7f7f", fontSize = "16px")),
    colGroup(name = paste0(last_year, " YTD Variance from Daily Avg Volume"), 
             columns = c("Mon_var", "Tue_var", "Wed_var", "Thu_var", "Fri_var"),
             headerStyle = list(color = "#d80b8c", fontSize = "16px"))
    ),
    columns = list(
...
no_show_ytd_site <- no_show_specialty %>%
  ...
  mutate(noShow_rate = round((`NoShow` + `SameDayCanceled`)/ Total,2),
         `noShow_rate_belowThirty` = round((`NoShow_belowThirty` + `SameDayCanceled_belowThirty`)/ `Total_belowThirty`,2),
         `noShow_rate_overThirty` = round((`NoShow_overThirty` + `SameDayCanceled_overThirty`)/ `Total_overThirty`,2)) %>%
  ...
  mutate(arrived_site = sum(Arrived),
         arrived_perc = round(Arrived/arrived_site,2)) 
...
reactable(
  ...
  columnGroups = list(
    ...
    colGroup(name = "No Show Rate - All", 
             columns = c("noShow_rate", "noShow_rate_belowThirty", "noShow_rate_overThirty"),
             headerStyle = list(color = "#7f7f7f", fontSize = "16px")),
    colGroup(name = "No Show Rate - Established", 
             columns = c("noShow_rate_Established", "noShow_rate_belowThirty_Established", "noShow_rate_overThirty_Established"),
             headerStyle = list(color = "#d80b8c", fontSize = "16px")),
    colGroup(name = "No Show Rate - New", 
             columns = c("noShow_rate_New","noShow_rate_belowThirty_New", "noShow_rate_overThirty_New"),
...
<span style='font-size: 20px; color: #212070'>-- **No Show Rate by Specialty**</span><br/>
...
### MSBI - No Shows {.tabset .tabset-fade .tabset-pills}
#### `r current_year` YTD
  ...
  columns = list(
    ...
    `noShow_rate_New` = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return (totalNoShow_New + totalSameDayCanceled_New) / totalTotal_New || 0
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
    ...
    `noShow_rate` = colDef(
      ...
      aggregate = JS("function(values, rows) {
        ...
        return (totalNoShow + totalSameDayCanceled) / totalTotal || 0
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
...
reactable(
  no_show_ytd_last_function("MSDMG"),
