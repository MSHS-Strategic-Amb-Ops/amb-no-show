...
# Add Missing ApptStatusFinal
  nms <- c("Campus", "Campus.Specialty.Group", "Appt.Year", "Appt.MonthYear", "New.PT2",
          ...
          "SameDayCanceled", "SameDayCanceled_sameDay", "SameDayCanceled_belowFourteen", "SameDayCanceled_belowThirty", "SameDayCanceled_overThirty")   # Vector of columns you want in this data.frame
...
```{r TEST No Show Calculation, echo = FALSE, warning = FALSE, message = FALSE}
...
#          `noShow_rate_sameDay` = round((`NoShow_sameDay` + `SameDayCanceled_sameDay`)/ `Total_sameDay`,2),
#          `noShow_rate_belowFourteen` = round((`NoShow_belowFourteen` + `SameDayCanceled_belowFourteen`)/ `Total_belowFourteen`,2),
...
#          `noShow_rate_belowThirty` = round((`NoShow_belowThirty` + `SameDayCanceled_belowThirty`)/`Total_belowThirty`,2),
#          `noShow_rate_overThirty` = round((`NoShow_overThirty` + `SameDayCanceled_overThirty`)/`Total_overThirty`,2),
#          new_perc = round(Arrived_New/Arrived,2)) %>%
#   group_by(Campus) %>%
...
```{r TEST MSHS No Show Current Year Output, echo = FALSE, warning = FALSE, message = FALSE}

no_show_ytd_site <- no_show_specialty %>%
  filter(Appt.Year == current_year) %>%
  group_by(Campus, Campus.Specialty.Group, Appt.Source.New, New.PT2) %>%
  summarise(Arrived = sum(Arrived),
            `Arrived_sameDay` = sum(`Arrived_sameDay`),
            `Arrived_belowFourteen` = sum(`Arrived_belowFourteen`),
            `Arrived_belowThirty` = sum(`Arrived_belowThirty`),
...
no_show_ytd_mshs <- no_show_specialty %>%
  filter(Appt.Year == current_year) %>%
  group_by(Campus.Specialty.Group, Appt.Source.New, New.PT2) %>%
  summarise(Arrived = sum(Arrived),
            `Arrived_sameDay` = sum(`Arrived_sameDay`),
            `Arrived_belowFourteen` = sum(`Arrived_belowFourteen`),
            `Arrived_belowThirty` = sum(`Arrived_belowThirty`),
...
<span style='font-size: 20px; color: #212070'>**No Show Rate by Group**</span><br/>
...
```{r TEST MSHS No Show Last Year Output, echo = FALSE, warning = FALSE, message = FALSE}
